...
for ratX in ratio_X:
	...
	for ratY in ratio_Y:
		...
		assocAx = sns.heatmap(assocMatrix, vmin = 0, vmax = 1, ax = assocAx, xticklabels = ratio_X, yticklabels = ratioReverse, cbar = (countX + countY) == 0, cbar_ax = None if (countX + countY) else cbar_ax,square = True)
		...
		assocAx.set_title("MI:" + '%.2f'%MI + "  PC:" + '%.2f'%assocCorrelation, fontsize = 7, y = 0.9, x = 0.5)
